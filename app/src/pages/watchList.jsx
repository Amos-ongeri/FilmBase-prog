...
const WatchList = () => {
  ...
  return (
    <main ref={topRef} className="min-h-50 min-w-full text-background dark:text-foreground dark:bg-background bg-foreground">
      ...
      <section className="relative px-6 md:px-12 pt-16 pb-10 overflow-hidden">
        ...
        <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-6 animate-fade-in">
          <div>
            ...
            <h1 className="text-4xl md:text-6xl font-extrabold leading-[1.05] mb-4 text-background dark:text-foreground">
              Watch <span className="text-primary">Later</span>
            </h1>
            <p className="text-muted/60 dark:text-muted-foreground max-w-xl">
          ...
          <div className="flex gap-3">
            <div className="glass rounded-2xl px-5 py-4 min-w-30">
              <div className="text-xs uppercase tracking-wider text-muted/60 dark:text-muted-foreground mb-1">Saved</div>
              <div className="text-2xl font-bold">{items?.length}</div>
            </div>
            <div className="glass rounded-2xl px-5 py-4 min-w-30">
              <div className="text-xs uppercase tracking-wider text-muted/60 dark:text-muted-foreground mb-1">Runtime</div>
      ...
      {/* Filters & sort bar */}
      <section className="px-6 md:px-12 sticky top-0 z-20 text-background dark:bg-background/80 backdrop-blur-xl py-4 border-y dark:border-border/50 border-muted/50">
        <div className="flex flex-wrap lg:flex-row gap-3 items-center lg:items-center">
          {/* Search */}
          <div className="relative flex-1 min-w-55">
            <Search className="absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4 dark:text-muted-foreground" />
            <Input
              value={query}
              onChange={(e) => setQuery(e.target.value)}
              placeholder="Search your watch list…"
              className="pl-10 h-11 rounded-full dark:bg-secondary/60 dark:border-border border-muted/50 focus-visible:ring-primary"
            />
          </div>

          {/* Genre */}
          <Select>
            <SelectTrigger className="h-11 w-fit lg:w-40 rounded-full dark:bg-secondary/60 dark:border-border border-muted/50">
          ...
          <div className="glass rounded-full p-1 flex items-center">
            <button
              ...
              className={`h-9 w-9 rounded-full flex items-center justify-center transition ${view === "grid" ? "bg-primary text-primary-foreground shadow-glow" : "text-muted-foreground hover:text-foreground"}`}
            ...
            <button
              ...
              className={`h-9 w-9 rounded-full flex items-center justify-center transition ${view === "list" ? "bg-primary text-primary-foreground shadow-glow" : "text-muted-foreground hover:text-foreground"}`}
        ...
        {hasActiveFilters && (
          <div className="flex flex-wrap items-center gap-2 mt-3">
            <span className="text-xs uppercase tracking-wider dark:text-muted-foreground mr-1">Active:</span>
      ...
      <section className="px-6 md:px-12 py-10">
        <div className="flex items-center justify-between mb-6">
          <div className="text-sm text-background dark:text-muted-foreground">
            Showing <span className="dark:text-foreground font-semibold">{filtered?.length}</span> of {items?.length} films
          </div>
        </div>
...
const EmptyState = ({ onClear }) => (
  <div className="glass rounded-3xl py-20 px-6 text-center max-w-xl mx-auto animate-fade-in">
    ...
    <p className="text-sm text-muted/60 dark:text-muted-foreground mb-6">Try loosening up the genre, platform, or rating to see more.</p>
    <Button onClick={onClear} className="bg-primary hover:opacity-90 text-foreground dark:text-primary-foreground rounded-full px-6 shadow-glow">
      Clear filters
    </Button>
  </div>
);

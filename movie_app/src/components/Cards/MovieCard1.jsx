...
const MovieCard1 = ({data,Key}) => {
    ...
    return(
        <div key={Key}  className="w-55 h-80 rounded-lg transition-shadow duration-200 space-y-2">
            <div className="h-[90%] w- relative group perspective-near overflow-hidden rounded-lg">
                <img onClick={navigate} loading="lazy" className="rounded-lg w-60 h-full object-cover object-center group-hover:translate-z-3 hover:opacity-70 transition-all duration-200 cursor-pointer" src={data.poster_path ? `https://image.tmdb.org/t/p/w500${data.poster_path}` : ''} />

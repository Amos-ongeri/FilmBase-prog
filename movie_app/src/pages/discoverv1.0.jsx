              ...
              <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                {GENRES.map((g) => (
                  <button
                    key={g}
                    onClick={() => setGenre(g)}
                    className="h-15 rounded-2xl overflow-hidden border border-border group    hover:border-0 transition duration-150"
                  >
          ...
          <div>
            <div className="flex items-center justify-between mb-6">
              <div className="text-sm text-muted-foreground">
                {query ? <> for "<span className="text-foreground">{query}</span>"</> : genre ? <> for "<span className="text-foreground">{genre}</span>"</> : type ? <> for "<span className="text-foreground">{type}</span>"</> : ""}
              </div>
              {(genre !== "All" || type !== "all") && (
                <button
